...
import SightingOfAComplaint from './SightingOfAComplaint';
...
const ViewMissingsPersons = ({ modal_visible, close_modal, data_complaint, is_home }) => {
    ...
    const [seeSightings, setSeeSightings] = useState(false)
    ...
    if (seeSightings) { // ver avistamientos de una denuncia, pasandole una denuncia_id
        return (
            <Modal
                transparent={false}
                visible={modal_visible}
                animationType="slide"
                onRequestClose={() => setSeeSightings(false)}
            >
                <View style={styles.modalContainer}>
                    <SightingOfAComplaint />
                </View>
            </Modal>
        )
    }
                                ...
                                <View style={{ flex: 5 }}>
                                    ...
                                    {is_home == true ?
                                        ...
                                        <TouchableOpacity onPress={() => { console.log('reportar avistamiento '), setSeeSightings(true)}}
                                            style={{ padding: 10, backgroundColor: Colors.RED, borderRadius: 16 }}
                                        >
                                            <Text style={{ textAlign: 'center', color: Colors.WHITE, fontSize: 18, fontWeight: '700' }} >Ver avistamientos</Text>
                                        </TouchableOpacity>
                                    }
                                </View>
                            </View>
